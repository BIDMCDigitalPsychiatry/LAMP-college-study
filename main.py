...
def trial_worker(participant_id, study_id, days_since_start_trial):
    ...
    if days_since_start_trial < 0: #if in trial period, don't do anyting
        ...
        if len(trial_scores) < len(trial_surveys) or gps_df['value'].mean() < GPS_SAMPLING_THRESHOLD:
            ...
            log.info(f"mailto:{SUPPORT_EMAIL}", f"Data Quality Threshold: Participant {participant_id} \n This participant did not meet data quality threshold in the trial period  (days_since_start = {str(days_since_start_trial)}). Please discontinue.")
